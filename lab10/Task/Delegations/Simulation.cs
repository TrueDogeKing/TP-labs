...
public class Simulation
{
    ...
    public async Task<SimulationResult> Run()
    {
        ...
        for (int day = 0; day < simulationDays && running; day++)
        {
            ...
            while (!phase1Completed)
            {
                try
                {
                    ...
                    phase1Completed = phase1Barrier.SignalAndWait(100); // 100ms timeout

                    if (!phase1Completed && _token.IsCancellationRequested)
                    {
                        _log?.Invoke($"[Thread {threadNumber}] Cancellation requested during Phase 1 barrier");
                        return new SimulationResult
                        {
                            ThreadNumber = threadNumber,
                            Healthy = 0,
                ...
                {
                    // Handle barrier post-phase exception if needed
                    break;
                }
            }
            ...
            while (!phase2Completed)
            {
                ...
                {
                    // Wait with a small timeout to allow cancellation checks
                    phase2Completed = phase2Barrier.SignalAndWait(100); // 100ms timeout

                    if (!phase2Completed && _token.IsCancellationRequested)
                    {
                        _log?.Invoke($"[Thread {threadNumber}] Cancellation requested during Phase 2 barrier");
                        return new SimulationResult
                        {
                            ThreadNumber = threadNumber,
                            Healthy = 0,
